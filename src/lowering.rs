...
#[cfg(test)]
mod tests {
    //! Feature #055 — tuple pattern matching in `when` arms.
    //!
    //! Verifies that the Phase 0c lowering rewrites
    //! `when X { { :ok v } -> body, { :err _ } -> body2 }` into
    //! `let __wtp_<id> = X; when __wtp_<id>.0 { :ok -> { let v = __wtp_<id>.1; body } ... }`.
